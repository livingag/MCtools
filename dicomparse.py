...
def BEAMGenerate(plandir, tempdir, EGS_HOME, beamind=[]):
    ...
    for x in beamind:
        ...
        if (
            'kv' not in plan.BeamSequence[x].BeamName.lower() and
            'ELECTRON' not in plan.BeamSequence[x].RadiationType
           ):
            ...
            if 'DYNAMIC' in plan.BeamSequence[x].BeamType:
                ...
                if glob(plandir + '/*.dlg'):
                    ...
                    VMATmlc.VMATdynamlcwrite(
                        plandir + '/' + mlcfile, bankA, bankB
                    )
                else:
                ...
                outfile = re.sub('!!MLC',
                                 EGS_HOME+'/dosxyznrc/' + mlcfile,
                                 outfile)

                outfile = re.sub('!!modemlc', '1', outfile)

                outfile = re.sub(
                    '!!name', '%s Beam %i' % (ID, beam.number), outfile
                )
                outfile = re.sub('!!brem', '%.3f' % (brem), outfile)
                outfile = re.sub('!!rand', '%i, %i' %
                                 (random() * 31328, random() * 30081), outfile)
                ...
                if beam.wedges > 0:
                    ...
                    EDW.EDWwrite(edw, plandir + '/' + filename,
                                 beam, wedge.WedgeID[-1:], xjawloc, yjawloc)
    ...
    for x in beamind:
        ...
        if (
            ...
            if 'DYNAMIC' in plan.BeamSequence[x].BeamType:
                ...
                outfile = open(EGS_HOME+'/templates/DOSXYZ_template_VMAT.egsinp', 'r').read()
                ...
                if glob(plandir + '/*.dlg'):
                    ...
                    for cp in beam.control:
                        ...
                        theta = np.degrees(
                            np.arccos(-np.sin(couch)*np.sin(gantry))
                        )
                        phi = np.degrees(
                            np.arctan2(
                                -np.cos(gantry), np.cos(couch)*np.sin(gantry)
                            )
                        )
                        coll = np.degrees(
                            3*np.pi/2-collimator-np.arctan2(
                                -np.sin(couch)*np.cos(gantry), np.cos(couch)
                            )
                        )
                ...
                outfile = re.sub(
                    '!!name',
                    '{0} {1} {2}'.format(ID, btype, beam.number),
                    outfile
                )
                outfile = re.sub(
                    '!!CT',
                    EGS_HOME+'/dosxyznrc/{}_CT.egsphant'.format(ID),
                    outfile
                )
                outfile = re.sub(
                    '!!BEAM',
                    '{}_{}{}_BEAM'.format(ID, btype, beam.number),
                    outfile
                )
                outfile = re.sub(
                    '!!rand',
                ...
                if glob(plandir + '/*.dlg'):
                    ...
                    outfile = re.sub('!!ncp', '{}'.format(
                        plan.BeamSequence[x].NumberOfControlPoints), outfile)

            else:

                beam = Beam(plan.BeamSequence[x])

                outfile = open(EGS_HOME+'/templates/DOSXYZ_template.egsinp', 'r').read()
                ...
                outfile = re.sub(
                    ...
                    '%.2f' % (
                        np.degrees(
                            3*np.pi/2-(beam.collimator+addcoll)-np.arctan2(
                ...
                outfile = re.sub(
                    ...
                    '%.2f' % (
                        np.degrees(
                            np.arccos(-np.sin(beam.couch)*np.sin(beam.gantry+addgantry))
                        )
                    ),
                    outfile
                )
                ...
                outfile = re.sub('!!name',
                                 ...
                                 outfile)
                outfile = re.sub('!!CT',
                                 EGS_HOME+'/dosxyznrc/%s_CT.egsphant' % (ID),
                                 outfile)
                outfile = re.sub('!!BEAM',
                                 '%s_Beam%i_BEAM' % (ID, beam.number),
                                 outfile)
                outfile = re.sub('!!rand',
                                 '%i, %i' % (
                                     random() * 31328, random() * 30081
                                 ),
                                 outfile)
...
def getBackscatter(plandir, beamind=[]):
    ...
    numbeams = len(plan.BeamSequence)
    ...
    for x in beamind:
        ...
        if (
            ...
            xjawloc, yjawloc, rmlc, mlcd = getMachineInfo(
                plan.BeamSequence[x].ControlPointSequence[0].NominalBeamEnergy, tempdir
            )
...
def calcBackscatter(x1, x2, y1, y2):
    ...
    Rx = (0.8-0.0187*(x1+x2))*(0.0395*(y1+y2)-0.0000355*(y1**3+y2**3))
    ...
    def __init__(self, beam, xjawloc=None, yjawloc=None, rmlc=None, mlcd=None):
        ...
        if xjawloc:
            ...
            self.xout = '%.3f, %.3f, %.3f, %.3f, ' % (
                self.x2[0], self.x2[1], self.x1[0], self.x1[1])
        if yjawloc:
            ...
            self.yout = '%.3f, %.3f, %.3f, %.3f, ' % (
                self.y1[0], self.y1[1], self.y2[0], self.y2[1]
            )
        if rmlc:
            try:
                self.mlc = np.asarray(
                    control[0x300a, 0x011a][2][0x300a, 0x011c].value
                )*0.1
