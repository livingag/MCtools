...
def ctcreate(EGS_HOME, ctdir='.', structs={}):
    ...
    iZ = pydicom.dcmread(dosefile).ImagePositionPatient[2]

    dose = DicomGrid(dosefile)
    dose.nz = len(dose.dicom.GridFrameOffsetVector)
    dose.dz = (dose.dicom.GridFrameOffsetVector[1] -
               dose.dicom.GridFrameOffsetVector[0])

    zdim = (np.ones((1, dose.nz+1))*dose.z0 -
            0.5*dose.dz +
            np.arange(0, dose.nz+1)*dose.dz)/10
    zdim.sort()

    zct = (np.asarray(dose.dicom.GridFrameOffsetVector) +
    ...
    if all(x not in rp.Manufacturer for x in ['Varian']):
        extid = [
            v for (k, v) in contours.items() if 'external' in k.lower()
        ]
        ext = rs.ROIContourSequence[extid[0]]
        try:
            couchid = [
                v for (k, v) in contours.items() if 'couch' in k.lower()
            ][0]
            couch = rs.ROIContourSequence[couchid]
            couchcont = np.array(
                couch.ContourSequence[0].ContourData
            ).reshape((-1, 3))[:, :2]
            ...
            for sli in couch.ContourSequence:
                ...
                if pts[:, 1].max() > couchmin: couchmin = pts[:, 1].max()
                if pts[:, 0].min() < couchxmin: couchxmin = pts[:, 0].min()
                if pts[:, 0].max() > couchxmax: couchxmax = pts[:, 0].max()
        except:
        ...
        try:
            ...
            extid = [
                v for (k, v) in contours.items() if 'body' in k.lower()
            ][0]
        ...
        try:
            ...
            couchintcont = np.array(
                couchint.ContourSequence[0].ContourData
            ).reshape((-1, 3))[:, :2]

            couchsurfcont = np.array(
                couchsurf.ContourSequence[0].ContourData
            ).reshape((-1, 3))[:, :2]
    ...
    for (k, v) in contours.items():
        if any(x in k.lower() for x in ['bolus','wire','wax','artifact','artefact']):
            structs[k] = 1
    ...
    for x, file in enumerate(files):
        ...
        if ctz in contslices.keys():
            for contind in contslices[ctz]:
                contour = np.array(
                    ext.ContourSequence[contind].ContourData
                ).reshape((-1, 3))[:, :2]
                cpath = Path(contour)
        ...
        try:
            if all(x not in rp.Manufacturer for x in ['Varian', 'Tomo']):
                ...
                ct.image[(cintinpath == False) &
                            (csurfinpath == True)] = -300
                planct[x, :, :] = ct.image
        except:
            pass

    interpct = interpolate.interp1d(slices, planct, axis=0, bounds_error=False,
                                    fill_value=-1000)(zct)
    ...
    if couchmin > dose.yy.max():
        ...
        dose.ny += rowadd
        dose.yy = np.append(dose.yy,
                            np.arange(1, rowadd+1)*dose.dy+dose.yy[-1])

    if couchxmin < dose.xx.min():
        coladd = np.ceil((dose.xx.min() - couchxmin) / dose.dx)
        dose.nx += coladd
        newx = np.arange(1, coladd+1)*dose.dy+(dose.xx[0]-coladd*dose.dx-dose.dx)
    ...
    if couchxmax > dose.xx.max():
        ...
        dose.nx += coladd
        dose.xx = np.append(dose.xx,
                            np.arange(1, coladd+1)*dose.dx + dose.xx[-1])

    xdim = (np.ones((1, int(dose.nx)+1))*dose.x0-0.5*dose.dx +
            np.arange(0, int(dose.nx)+1)*dose.dx)/10

    ydim = (np.ones((1, int(dose.ny)+1))*dose.y0-0.5*dose.dy +
            np.arange(0, int(dose.ny)+1)*dose.dy)/10
    ...
    with open(ctdir+'/'+'%s_CT.egsphant' % (ID), 'wb') as outfile:
        ...
        for x in np.arange(interpct.shape[0]):
            ...
            newct = f(dose.xx, dose.yy) # * ct.dicom.RescaleSlope + ct.dicom.RescaleIntercept
